...
<head>
    ...
    <meta name="keywords" content="时代民芯">
    <meta name="description" content="时代民芯">
...
<script>
    ...
    $(document).ready(function(){
    	loadNotice();
    	loadSms();
    	setInterval("loadTodoNum();",10000);
    	setInterval("loadModuleTodoNum();",10000);
    	setInterval("loadJianyanTodoNum();",10000);
    });
    ...
    function loadTodoNum(){
   	 //加载外协待处理数
       var url = "subcontractApplyController.do?todonum";
       jQuery.ajax({
           url:url,
           type:"GET",
           dataType:"JSON",
           ...
           success:function(data){
               if(data.success){
                   //加载待处理条数
                   $("#waixietodonum").html(data.obj);
                   if(data.obj>0){
                       $("#waixietodonum").css("color","red");
                   }else{
                       $("#waixietodonum").removeAttr("style");
                   }
               }
           }
       });
   }
    function loadModuleTodoNum(){
    	//加载模块待处理数
          var url = "modulesubcontractApplyController.do?todonum";
          jQuery.ajax({
              url:url,
              type:"GET",
              dataType:"JSON",
              ...
              success:function(data){
                  if(data.success){
                      //加载公告条数
                      $("#moduletodonum").html(data.obj);
                      if(data.obj>0){
                          $("#moduletodonum").css("color","red");
                      }else{
                          $("#moduletodonum").removeAttr("style");
                      }
                  }
              }
          });
      }
    function loadJianyanTodoNum(){
    	//加载验收待处理数
          var url = "subcontractInspectionController.do?todonum";
          jQuery.ajax({
              url:url,
              type:"GET",
              dataType:"JSON",
              ...
              success:function(data){
                  if(data.success){
                      //加载公告条数
                      $("#jianyantodonum").html(data.obj);
                      if(data.obj>0){
                          $("#jianyantodonum").css("color","red");
                      }else{
                          $("#jianyantodonum").removeAttr("style");
                      }
                  }
              }
          });
