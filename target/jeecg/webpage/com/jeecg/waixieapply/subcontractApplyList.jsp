...
<body>
	...
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','subcontractApplyController.do?goAdd','subcontractApplyList','100%','100%')" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','subcontractApplyController.do?goUpdate','subcontractApplyList','100%','100%')" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','subcontractApplyController.do?doBatchDel','subcontractApplyList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>
			<button onclick="detail('查看','subcontractApplyController.do?goUpdate','subcontractApplyList','100%','100%')" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查看
			</button>
			<!-- <button onclick="ImportXls('导入','','subcontractApplyList',600,400)" id="btn_download" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span> 导入
			</button> -->
			<button onclick="ExportXls('导出','','subcontractApplyList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出Excel
			</button>
			<!-- <button onclick="ExportXlsByT('模板下载','','subcontractApplyList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
		...
		<div class="table-responsive">
			...
			<table id="subcontractApplyList"></table>
		</div>
	</div>
	<script type="text/javascript">
		...
		$(function() {
			...
			promiseArr.push(new Promise(function(resolve, reject) {
				initDictByCode(subcontractApplyListdictsData,"dwmc",promiseArr,"subcontract_supplier","dwmc");
			}));
			promiseArr.push(new Promise(function(resolve, reject) {
			...
			$('#subcontractApplyList').on(
					'check.bs.table uncheck.bs.table load-success.bs.table '
							+ 'check-all.bs.table uncheck-all.bs.table',
					function() {
						$('#btn_delete').prop('disabled',!$('#subcontractApplyList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#subcontractApplyList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#subcontractApplyList').bootstrapTable({
									url : 'subcontractApplyController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#subcontractApplyList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#subcontractApplyList').bootstrapTable('getOptions').pageNumber;
												...
												formatter : function(value,row, index) {
													...
													if(row.confirmState==1){
														href += "<a href='javascript:void(0);'  class='ace_button'  onclick=confirmtask('审核','subcontractApplyController.do?goConfirm&id="
															+ row.id	
														+ "','subcontractApplyList','100%','100%')> <i class='fa fa-trash-o' aria-hidden='true'></i> ";
														href += "审核</a>&nbsp;";
													}
													if(row.completeState==1){
														href += "<a href='javascript:void(0);'  class='ace_button' onclick=copObj('subcontractApplyController.do?doComplete&id="
															+ row.id
															+ "','subcontractApplyList')>  <i class='fa fa-trash-o' aria-hidden='true'></i> ";
														href += "确认完成</a>&nbsp;";
													}
		...
		function reloadTable() {
			$('#subcontractApplyList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'subcontractApplyController.do?upload',"subcontractApplyList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("subcontractApplyController.do?exportXls","subcontractApplyList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("subcontractApplyController.do?exportXlsByT","subcontractApplyList");
		}
