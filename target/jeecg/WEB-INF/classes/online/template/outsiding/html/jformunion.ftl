...
<head>
 ...
 <body>
  <form id="formobj" action="${basePath}/cgFormBuildController.do?saveOrUpdateMore" name="formobj" method="post"><input type="hidden" id="btn_sub" class="btn_sub"/>
	<#include "online/template/outsiding/html/jformhead.ftl">
...
<!-- update-end-Author:zhangweijian  Date: 20180709 for：#2919 online样式问题 -->
	...
	<#list subtablelist as sub >
	    <#if field['${sub}']?exists >
		    ...
		    <#include "online/template/outsiding/html/jformonetoone.ftl">
		    <#else>
		    <#include "online/template/outsiding/html/jformonetomany.ftl">
		    </#if>
		</#if>
	</#list>
</div>
		...
		<#list subtablelist as sub >
		    <#if field['${sub}']?exists >
				<#if field['${sub}'].head.relationType!=1 >
			    <#include "online/template/outsiding/html/jformonetomanytpl.ftl">
			    </#if>
			</#if>
		</#list>
		</table>
