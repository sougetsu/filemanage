...
<html>
 ...
 <body>
  ...
  <input type="hidden" id="btn_sub" class="btn_sub"/>
	<#include "online/template/subcontract/html/jformhead.ftl">
	
			
	
	<div align="center" style="padding-top:8px;display: none;" id = "sub_tr"> 
...
<script type="text/javascript">
   $(function(){
	...
	if(location.href.indexOf("goAddButton.do")!=-1){
		//其他模式显示提交按钮
		$("#sub_tr").show();
	}
   });
  ...
  function uploadFile(data){
	...
	function del(url,obj){
			...
			<div style="width: auto;height: 200px;">
				...
				<#list subtablelist as sub >
				    <#if field['${sub}']?exists >
					    ...
					    <#include "online/template/subcontract/html/jformonetoone.ftl">
					    <#else>
					    <#include "online/template/subcontract/html/jformonetomany.ftl">
					    </#if>
					</#if>
				</#list>
				</div>
		...
		<#else>
			<script type="text/javascript">
				$(function(){
					$("#formobj").Validform({tiptype:function(msg,o,cssctl){if(o.type == 3){layer.open({title:'提示信息',content:msg,icon:5,shift:6,btn:false,shade:false,time:5000,cancel:function(index){o.obj.focus();layer.close(index);},yes:function(index){o.obj.focus();layer.close(index);},})}},btnSubmit:"#btn_sub",btnReset:"#btn_reset",ajaxPost:true,usePlugin:{passwordstrength:{minLen:6,maxLen:18,trigger:function(obj,error){if(error){obj.parent().next().find(".Validform_checktip").show();obj.find(".passwordStrength").hide();}else{$(".passwordStrength").show();obj.parent().next().find(".Validform_checktip").hide();}}}},callback:function(data){if(data.success==true){uploadFile(data);}else{if(data.responseText==''||data.responseText==undefined){$.messager.alert('错误', data.msg);$.Hidemsg();}else{try{var emsg = data.responseText.substring(data.responseText.indexOf('错误描述'),data.responseText.indexOf('错误信息')); $.messager.alert('错误',emsg);$.Hidemsg();}catch(ex){$.messager.alert('错误',data.responseText+'');}} return false;}if(!neibuClickFlag){var win = frameElement.api.opener; win.reloadTable();}}});
				});
			</script>
			</form>
		</#if>
		...
		<#list subtablelist as sub >
		    <#if field['${sub}']?exists >
				<#if field['${sub}'].head.relationType!=1 >
			    <#include "online/template/subcontract/html/jformonetomanytpl.ftl">
			    </#if>
			</#if>
		</#list>
		</table>
