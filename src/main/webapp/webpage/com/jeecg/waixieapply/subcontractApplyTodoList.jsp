...
<body>
	...
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','subcontractApplyController.do?goAdd','subcontractApplyTodoList','100%','100%')" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','subcontractApplyController.do?goUpdate','subcontractApplyTodoList','100%','100%')" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','subcontractApplyController.do?doBatchDel','subcontractApplyTodoList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>
			<button onclick="detail('查看','subcontractApplyController.do?goUpdate','subcontractApplyTodoList','100%','100%')" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查看
			</button>
			<!-- <button onclick="ImportXls('导入','','subcontractApplyTodoList',600,400)" id="btn_download" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span> 导入
			</button> -->
			<button onclick="ExportXls('导出','','subcontractApplyTodoList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出Excel
			</button>
			<!-- <button onclick="ExportXlsByT('模板下载','','subcontractApplyTodoList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
		...
		<div class="table-responsive">
			...
			<table id="subcontractApplyTodoList"></table>
		</div>
	</div>
	<script type="text/javascript">
		...
		var subcontractApplyTodoListdictsData = {};
		$(function() {
			var promiseArr = [];
			promiseArr.push(new Promise(function(resolve, reject) {
				initDictByCode(subcontractApplyTodoListdictsData,"id",promiseArr,"subcontract_plan","id");
			}));
			promiseArr.push(new Promise(function(resolve, reject) {
				initDictByCode(subcontractApplyTodoListdictsData,"id",promiseArr,"t_s_depart","departname");
			}));
			promiseArr.push(new Promise(function(resolve, reject) {
				initDictByCode(subcontractApplyTodoListdictsData,"dwmc",promiseArr,"subcontract_supplier","dwmc");
			}));
			promiseArr.push(new Promise(function(resolve, reject) {
				initDictByCode(subcontractApplyTodoListdictsData,"bpm_status",promiseArr,"","typename");
			}));
			...
			$('#subcontractApplyTodoList').on(
					'check.bs.table uncheck.bs.table load-success.bs.table '
							+ 'check-all.bs.table uncheck-all.bs.table',
					function() {
						$('#btn_delete').prop('disabled',!$('#subcontractApplyTodoList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#subcontractApplyTodoList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#subcontractApplyTodoList').bootstrapTable({
									url : 'subcontractApplyController.do?tododatagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#subcontractApplyTodoList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#subcontractApplyTodoList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													if(row.confirmState==1){
														href += "<a href='javascript:void(0);'  class='ace_button'  onclick=confirmtask('审核','subcontractApplyController.do?goConfirm&id="
															+ row.id	
														+ "','subcontractApplyTodoList','100%','100%')> <i class='fa fa-trash-o' aria-hidden='true'></i> ";
														href += "审核</a>&nbsp;";
													}
													if(row.completeState==1){
														href += "<a href='javascript:void(0);'  class='ace_button' onclick=copObj('subcontractApplyController.do?doComplete&id="
															+ row.id
															+ "','subcontractApplyTodoList')>  <i class='fa fa-trash-o' aria-hidden='true'></i> ";
														href += "确认完成</a>&nbsp;";
													}
													href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('subcontractApplyController.do?doDel&id="
														+ row.id
														+ "','subcontractApplyTodoList')>  <i class='fa fa-trash-o' aria-hidden='true'></i> ";
													href += "删除</a>&nbsp;";
													return href;
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,sysOrgCode,status,bpmStatus,applybh,planno,xmmc,sqbm,snxh,kth,xmfzr,wxjgdw,dz,lxr,dh,cz,sl,bh,sm,applydate,completedate,attach,wtlr,jtyq,bz,bmyj,bmldyj,gsldyj,urgentdate,cost,'
				};
		...
		function reloadTable() {
			$('#subcontractApplyTodoList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'subcontractApplyController.do?upload',"subcontractApplyTodoList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("subcontractApplyController.do?exportXls","subcontractApplyTodoList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("subcontractApplyController.do?exportXlsByT","subcontractApplyTodoList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = subcontractApplyTodoListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = subcontractApplyTodoListdictsData[dictKey];
